...
class Testget_employee_skill_data:
    @pytest.mark.Get_Employee_Skill_Data_test
    def test_get_employee_skill_data(self):
        LOGGER = logging.getLogger(__name__)
        LOGGER.info("Starting the Testcase of TestGetEmployeeSkillData Test")
        # now returns both
        response, response_json = get_request(
            url=APIConstants().Get_Employee_Skill_Data_url(),
            auth=None,
            headers=None,
            in_json=True
        )
        LOGGER.info("GET Get_Employee_Skill_Data.")
