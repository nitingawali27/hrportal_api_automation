...
class TestEmployeeCertificates:
    @pytest.mark.Get_Employee_Certificates_test
    def test_Get_Employee_Certificates_url(self):
        LOGGER = logging.getLogger(__name__)
        LOGGER.info("Starting the Testcase of TestGetEmployeeCertificates Test")
        # now returns both
        response, response_json = get_request(
            url=APIConstants().Get_Employee_Certificates_url(),
            auth=None,
            headers=None,
            in_json=True
        )
        LOGGER.info("GET Get_Employee_Certificates details.")
        ...
        LOGGER.info("Testcase of TestGetEmployeeCertificates is Done")
