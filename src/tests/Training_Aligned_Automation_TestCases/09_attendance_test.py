...
class TestEmployeeAttendance:
    @pytest.mark.Get_Employee_Attendance_test
    def test_get_employee_attendance(self):
        LOGGER = logging.getLogger(__name__)
        LOGGER.info("Starting the Testcase of TestGetEmployeeAttendance Test")
        # now returns both
        response, response_json = get_request(
            url=APIConstants().Get_Employee_Attendance_url(),
            auth=None,
            headers=None,
            in_json=True
        )
        LOGGER.info("GET Get_Employee_Attendance details.")
        ...
        LOGGER.info("Testcase of TestGetEmployeeAttendance is Done")
