...
class TestGetEmployeeDetails:
    @pytest.mark.Get_Employee_Details_test
    def test_Get_Employee_Details(self):
        LOGGER = logging.getLogger(__name__)
        LOGGER.info("Starting the Testcase of TestGetEmployeeDetails")
        # now returns both
        response, response_json = get_request(
            url=APIConstants().Get_Employee_Details_url(),
            auth=None,
            headers=None,
            in_json=True
        )
        LOGGER.info("GET Get_Employee_Details.")
