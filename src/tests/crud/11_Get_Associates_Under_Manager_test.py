...
class TestAssociatesUnderManager:
    @pytest.mark.Get_Associates_Under_Manager_test
    def test_Get_Associates_Under_Manager_url(self):
        LOGGER = logging.getLogger(__name__)
        LOGGER.info("Starting the Testcase of TestGetAssociatesUnderManager Test")
        # now returns both
        response, response_json = get_request(
            url=APIConstants().Get_Associates_Under_Manager_url(),
            auth=None,
            headers=None,
            in_json=True
        )
        LOGGER.info("GET Get_Associates_Under_Manager details.")
        ...
        LOGGER.info("Testcase of TestGetAssociatesUnderManager is Done")
