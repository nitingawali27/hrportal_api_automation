...
class Testgetreportingmanagerdetails:
    @pytest.mark.Get_Reporting_Manager_Details_test
    def test_get_reporting_manager_details(self):
        LOGGER = logging.getLogger(__name__)
        LOGGER.info("Starting the Testcase of TestGetReportingManagerDetails Test")
        # now returns both
        response, response_json = get_request(
            url=APIConstants().Get_Reporting_Manager_Details_url(),
            auth=None,
            headers=None,
            in_json=True
        )
        LOGGER.info("GET Get_Reporting_Manager_Details.")
        ...
        LOGGER.info("Testcase of TestGetReportingManagerDetails is Done")
